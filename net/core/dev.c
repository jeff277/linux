...
static inline void ____napi_schedule(struct softnet_data *sd,
				     struct napi_struct *napi)
{
	// 这是网卡硬中断中的处理逻辑, 将napi_struct 挂载到 softnet_data的poll_list.  
...
static int __init net_dev_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		struct softnet_data *sd = &per_cpu(softnet_data, i);  // [网络初始化 - 2] 为每个CPU初始化softnet_data。 softnet_data的含义Incoming packets are placed on per-CPU queues
...
#endif
		INIT_LIST_HEAD(&sd->poll_list);		// [网络初始化 - 2] 用于等待驱动程序将其poll函数注册进来.
		sd->output_queue_tailp = &sd->output_queue;
#ifdef CONFIG_RPS
	...
	open_softirq(NET_TX_SOFTIRQ, net_tx_action);  // [网络初始化 - 2] 为每个tx/rx软中断注册 中断处理函数.
	open_softirq(NET_RX_SOFTIRQ, net_rx_action);  // [网络初始化 - 2] 所以，也可以认为 net_rx_action就是网络子系统处理数据包的入口了
