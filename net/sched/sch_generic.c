...
void __qdisc_run(struct Qdisc *q)
{
	...
	while (qdisc_restart(q, &packets)) {
		...
		if (quota <= 0) {	
			// 没令牌啦(不能继续发包了)
			if (q->flags & TCQ_F_NOLOCK)
				set_bit(__QDISC_STATE_MISSED, &q->state);	// __QDISC_STATE_MISSED 表示队列可能需要在未来的时间点继续处理未完成的数据包
			else
				// 这是NET_RX_SOFTIRQ比NET_TX_SOFTIQR多的原因之一.(发包没有令牌了就会IRQ一次, 收包没有流控令牌一说)
				__netif_schedule(q);    // 将队列放入网络设备的调度队列中, 以便稍后继续发包. 要schedule一次的根本原因是避免长时间持有锁和CPU。 
